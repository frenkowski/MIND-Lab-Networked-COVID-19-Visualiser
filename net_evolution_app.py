...
reload(mind_tr)  # <-- da rimuovere in produzione
# 20200518 - Marco End
...
networks_dictionary = {}
networks_dictionary['Baseline'] = "Baseline"
networks_dictionary['sim1'] = 'sim1'
networks_dictionary['sim2'] = "sim2"
networks_dictionary['sim3'] = "sim3"
networks_dictionary['sim4'] = "sim4"
networks_dictionary['sim5'] = "sim5"

# net to load at first launch
current_nets = 'Baseline'

image_filename1 = Path(
    'images/{}_sim0.jpeg'.format(networks_dictionary[current_nets]))
image_filename2 = Path(
    'images/{}_sim1.jpeg'.format(networks_dictionary[current_nets]))
encoded_image1 = base64.b64encode(open(image_filename1, 'rb').read())
encoded_image2 = base64.b64encode(open(image_filename2, 'rb').read())

# load baseline
fp_in = Path(
    "network_dumps/{}.pickle".format(networks_dictionary[current_nets]))
nets = list()
with open(fp_in, "rb") as f:
    nets = pickle.load(f)

# load baseline history
history_path = "assets/network_history_{}.pickle".format(
    networks_dictionary[current_nets])
...
tab1_content = dbc.Container(
    [
                          ...
                          dbc.Row(
                              [
                                  dbc.Col([
                                      ...
                                      dbc.Spinner(html.Div(id="gif_id", children=[]),
                                                  color="primary"),
                                  ...
                                  dbc.Col([
                                      ...
                                      html.Ul(id='my-list',
                                              children=[html.Li(i) for i in modelParam]),
                                      html.P(
                              ...
                              [
                                  dbc.Col([
                                      ...
                                      dbc.Row(
                                          [
                                              dbc.Col(
                                                  [dbc.Button("<--- Back ", id="back")], width={"offset": 5}),
                                              # width={"size":3})
                                              dbc.Col(
                                                  [dbc.Button("Forward --->", id="forward")],)

                                          ]),
                                      html.Br(),
                                      dbc.Spinner(
                                          html.Div(id='div_spin'), color="primary"),

                                      dcc.Graph(id="graph_sim", style={
                                                'display': 'block'}),
                                      html.Div(style={'margin-top': '60px'}),
                                      ...
                                      html.Div(id="cyto-container", children=[
                                          mind_tr.Get_Grid_Div(
                                              app, "cyto_1", mind_tr.create_network_data(nets[0]))
                                      ])
                                      # 20200518 - Marco End
                                  ], md=12),
        ...
        dbc.Container(id="div_2day",  children=[
            ...
            html.Br(),
            dbc.Row([
                    dbc.Col([dbc.Spinner(dcc.Graph(id='img_day1', style={
                            'display': 'block'}), color="primary"), html.Br()], md=6),

                    dbc.Col([dbc.Spinner(dcc.Graph(id='img_day2', style={
                            'display': 'block'}), color="primary"), html.Br()], md=6),
            ...
            dbc.Row([
                    html.Br(),
                    dbc.Col([dbc.Spinner(dcc.Graph(id="graphImg1", style={
                            'display': 'block'}), color="primary"), html.Br()], md=6),
                    dbc.Col([dbc.Spinner(dcc.Graph(id="graphImg2", style={
                            'display': 'block'}), color="primary"), html.Br()], md=6),
                          ...
                          dbc.Row(
                              [
                                  dbc.Col([
                                      ...
                                      html.Br(),
                                      dbc.Spinner(html.Div(id='div_spin_tab3'),
                                                  color="primary"),

                                      dcc.Graph(id="graph_evolution_sim",
                                                style={'display': 'block'}),
                                      dcc.Graph(id="simulation_image_tab2",
                                                style={'display': 'block'}),
                                  ], md=12),
...
def update_graphics(day, max_slider):
    ...
    # plotly graph
    colors = {'S': '#0000ff', 'E': '#ffa300',
              'I': '#ff0000', 'D': '#000000', 'R': '#00ff00'}
    ...
    if day > 0:
        image_filename1 = Path(
            'images/{}_sim'.format(file_name) + str(day - 1) + '.jpeg')
        image_filename2 = Path(
            'images/{}_sim'.format(file_name) + str(day) + '.jpeg')
        day_prev = day - 1
    else:
    ...
    current = {
        'data': [
            {'x': [1], 'y': [s], 'type': 'bar', 'name': 'S',
                'marker': {'color': colors['S']}},
            {'x': [1], 'y': [e], 'type': 'bar', 'name': 'E',
                'marker': {'color': colors['E']}},
            {'x': [1], 'y': [i], 'type': 'bar', 'name': 'I',
                'marker': {'color': colors['I']}},
            {'x': [1], 'y': [r], 'type': 'bar', 'name': 'R',
                'marker': {'color': colors['R']}},
            {'x': [1], 'y': [d], 'type': 'bar', 'name': 'D',
    ...
    # 20200518 - Marco
    return [fig, [], src_image1, src_image2, prev, current, mind_tr.Get_Grid_Div(app, "cyto_" + str(day), mind_tr.create_network_data(G))]
...
def update_plot(day, new_max):
    ...
    graph = {'data': [
        {'x': x_axis, 'y': y_S, 'type': 'line',
            'name': 'S', 'marker': {'color': '#0000ff'}},
        {'x': x_axis, 'y': y_E, 'type': 'line',
            'name': 'E', 'marker': {'color': '#ffa300'}},
        {'x': x_axis, 'y': y_I, 'type': 'line',
            'name': 'I', 'marker': {'color': '#ff0000'}},
        {'x': x_axis, 'y': y_R, 'type': 'line',
            'name': 'R', 'marker': {'color': '#00ff00'}},
        {'x': x_axis, 'y': y_D, 'type': 'line',
            'name': 'D', 'marker': {'color': '#000000'}},
    ],
        'layout': {
